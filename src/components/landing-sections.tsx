...
export function HeroSection() {
  return (
    <section className="relative min-h-[455px] overflow-hidden border-b border-line">
      <Image
      ...
      />
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(9,10,13,0.98)_0%,rgba(9,10,13,0.9)_28%,rgba(9,10,13,0.42)_58%,rgba(9,10,13,0.18)_100%)]" />
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(9,10,13,0.15)_0%,rgba(9,10,13,0.08)_50%,rgba(9,10,13,0.84)_100%)]" />
      <div className="section-shell relative z-10 flex min-h-[455px] flex-col justify-center py-8">
        <div className="reveal max-w-[760px]">
          <div className="mb-4 flex items-center gap-5 text-xs font-medium uppercase tracking-wide text-muted">
            <span>Автомобили из Китая и Кореи под ключ</span>
            <span className="h-px w-16 bg-accent" />
          </div>
          <h1 className="font-heading text-[34px] font-black uppercase leading-[1.1] tracking-normal sm:text-5xl lg:text-[56px]">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <LeadModalButton className="gap-2" source="hero" icon="none">
              Рассчитать стоимость
            </LeadModalButton>
            <ButtonLink href="#cases" variant="secondary" className="gap-3">
              <PlayCircle size={19} />
              Смотреть видео
            </ButtonLink>
          </div>
        ...
        <div className="mt-6 grid max-w-[760px] gap-0 rounded-md border border-line bg-[#111318]/80 backdrop-blur sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            ["Гарантия юридической чистоты", "Проверка по 100+ параметрам", CheckCircle2],
            ["Сопровождение под ключ", "От подбора до постановки на учёт", Handshake]
          ].map(([title, text, Icon], index) => (
            <div
              key={String(title)}
              className="flex min-h-[78px] gap-3 border-line px-4 py-4 text-left sm:border-r last:border-r-0"
            >
              <Icon className="mt-1 shrink-0 text-white" size={24} strokeWidth={1.7} />
              <div>
                <div className="text-xs font-black text-white">{String(title)}</div>
                <div className="mt-1 text-[10px] leading-4 text-muted">{String(text)}</div>
              </div>
            </div>
          ))}
        </div>
    ...
    <section className="section-shell py-14">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {trust.map(({ icon: Icon, title, text }) => (
          <div key={title} className="dark-card rounded-lg p-5 transition hover:-translate-y-1 hover:border-accent/60">
    ...
    <section id="services" className="section-shell py-16">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {services.map(({ icon: Icon, title }) => (
          <div key={title} className="dark-card flex items-center gap-4 rounded-lg p-5">
...
export function ProcessSection() {
  return (
    <section id="process" className="border-y border-line bg-black/25 py-16">
      <div className="section-shell">
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-7">
          {process.map((step, index) => (
            <div key={step} className="relative rounded-lg border border-line bg-panel/70 p-5">
              <div className="font-heading text-3xl font-black text-accent">{String(index + 1).padStart(2, "0")}</div>
              <div className="mt-4 text-sm font-bold">{step}</div>
            </div>
          ))}
        </div>
      </div>
...
export function PopularCars({ cars }: { cars: CarType[] }) {
  return (
    <section id="catalog" className="section-shell py-16">
...
export function CalculatorSection() {
  return (
    <section id="calculator" className="section-shell grid gap-8 py-16 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
      <div>
        <SectionHeading
          eyebrow="Расчёт"
...
export function CasesSection() {
  return (
    <section id="cases" className="section-shell py-16">
      <SectionHeading
        eyebrow="Кейсы"
  ...
  return (
    <section id="faq" className="section-shell py-16">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        {faq.map((item) => (
          <details key={item.q} className="dark-card group rounded-lg p-5">
...
export function FinalCta() {
  return (
    <section className="section-shell py-16">
      <div className="dark-card overflow-hidden rounded-lg p-8 md:p-10">
